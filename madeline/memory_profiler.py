...
class MemoryProfiler:
    ...
    def compute_budget_bytes(self) -> int:
        ...
        The budget is computed as:
            ...
            - optimizer_state_estimate       (additional 20% of peak for
                                              optimizer state, grad buffers,
                                              and activation checkpoints that
                                              may not appear in the fwd-only
                                              peak watermark)
        """
        ...
        # peak watermark is captured.  Reserve an additional 20% of peak as a
        # conservative estimate so the cache does not crowd out the optimizer.
        optimizer_state_estimate = int(self._peak_memory * 0.20)
